...
const ConeManager = () => {
  ...
  const fetchCones = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/cones");
      setCones(res.data);
    } catch (err) {
  ...
  const handleAddCone = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("http://localhost:5000/api/cones", {
        color,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/cones/${id}`);
      fetchCones();
    } catch (err) {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:5000/api/cones/${editingConeId}`, {
        color,
        availableQuantity: Number(availableQuantity),
      });
