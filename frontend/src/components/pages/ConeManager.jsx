...
const ConeManager = () => {
  ...
  const fetchCones = async () => {
    try {
      const res = await axios.get("https://billing-software-4dft.onrender.com/api/cones");
      setCones(res.data);
    } catch (err) {
  ...
  const handleAddCone = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("https://billing-software-4dft.onrender.com/api/cones", {
        color,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://billing-software-4dft.onrender.com/api/cones/${id}`);
      fetchCones();
    } catch (err) {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`https://billing-software-4dft.onrender.com/api/cones/${editingConeId}`, {
        color,
        availableQuantity: Number(availableQuantity),
      });
